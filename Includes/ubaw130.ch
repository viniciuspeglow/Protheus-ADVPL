#ifdef SPANISH
	#define STR0001 'Error en el manejo del XML recibido.'
	#define STR0002 'Error en la devolución. ¡La Referencia/Marca es obligatoria!'
#else
	#ifdef ENGLISH
		#define STR0001 'Error manipulating received XML.'
		#define STR0002 'Error in return. Reference/brand is mandatory!'
	#else
		#define STR0001 'Erro na manipulação do Xml recebido.'
		#define STR0002 'Erro no retorno. A Referência/Marca é obrigatória!'
	#endif
#endif
